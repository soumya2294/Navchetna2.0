...
/**
 * Sends messages to FitMates AI Coach with fallback handling
 */
export async function sendAICoachMessage(messages) {
  const profile = getStoredProfile()

  // First attempt: Backend proxy route
  try {
    const res = await fetchApi('/api/ai/coach', {
      method: 'POST',
      body: JSON.stringify({
        messages,
        userProfile: profile
    ...
    if (res && res.reply) {
      return { reply: res.reply, model: res.model || 'Groq' }
    }
  } catch (proxyError) {
    console.warn('Backend proxy /api/ai/coach unavailable, falling back to direct Groq API...', proxyError.message)
  }

  // Second attempt / Fallback: Direct Groq API call
  const clientApiKey = import.meta.env.VITE_GROQ_API_KEY
  if (!clientApiKey) {
    throw new Error('Groq API key is not configured. Please set VITE_GROQ_API_KEY in client/.env')
  }
...
Title: ${profile.title || 'Beginner'}
Disciplines: ${Array.isArray(profile.disciplines) ? profile.disciplines.join(', ') : 'Fitness'}
Workout Streak: ${profile.streak || 0} days
Completed Workouts: ${profile.workouts || 0}
Active Days: ${profile.activeDays || 0}
--------------------------------------`
  }

  const payloadMessages = [
    { role: 'system', content: `${FITMATES_SYSTEM_PROMPT}${profileContext}` },
  ...
  let completion
  try {
    completion = await tryCall('openai/gpt-oss-120b')
  } catch (err1) {
    console.warn('Groq 120b failed, trying 20b fallback:', err1.message)
    completion = await tryCall('openai/gpt-oss-20b')
  }

  const reply = completion?.choices?.[0]?.message?.content || 'I could not process that workout query right now. Please try again.'
  return { reply, model: completion?.model || 'openai/gpt-oss' }
}
